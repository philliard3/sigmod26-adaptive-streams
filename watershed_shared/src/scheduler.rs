...
// other possible ways to group/deduplicate bins
// #[derive(Debug, Clone, Copy)]
    ...
    SendError: Debug,
{
    ...
    if ratio_kept < 1.0 {
        warn!(
            "ratio_kept was less than 1.0. we had to drop {:.2}% of the items",
            100.0 * (1.0 - ratio_kept)
        );
        budget *= ratio_kept;
    }
            ...
            match (target_pipe, *REROUTE_ZERO_OPTION) {
                ...
                (0, RerouteZeroOption::First) => {
                    if pipes.len() > 1 {
                        ...
                        warn!("REROUTE_ZERO_OPTION is set to First, but there is only one pipe. ignoring the variable and using pipe 0 as planned");
                        self.send_helper_past_data_zeroes(&tuples, bins, now, now_unix_ns);
                    }
                }
                (0, RerouteZeroOption::Random) => {
                    if pipes.len() > 1 {
                        ...
                        warn!("REROUTE_ZERO_OPTION is set to Random, but there is only one pipe. ignoring the variable and using pipe 0 as planned");
                        self.send_helper_past_data_zeroes(&tuples, bins, now, now_unix_ns);
                    }
                }
            ...
            while let Ok(data) = self.back_channel.try_recv() {
                for (tuple_id, pipeline_id, time_merged) in data {
                    if let Some(pending_data) = self.pending_data.remove(&tuple_id) {
                        ...
                        error!("pending data was not found for tuple received from back channel with id {}", tuple_id);
                    }
                }
            }
        }
                ...
                let num_future_items = match future_kind {
                    ...
                    FutureWindowKind::TimeWithMaximumCount {
                        ...
                        if average_time_per_item_micros < 100.0 {
                            ...
                            debug!("time_millis was {time_millis}, average_time_per_item_micros was {average_time_per_item_micros}, history_len was {history_len}, so we are forecasting {v} future items");
                            if v > max_count {
                                // we have to shrink down our forecast to account for this
                                ratio_kept = max_count as f64 / v as f64;
                                max_count
                            } else {
                                v
                            }
